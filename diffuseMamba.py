...
class DiMBlock(nn.Module):
    ...
    def __init__(self, hidden_size:int, d_conv:int, layer_idx:int, mlp_ratio=4.0, **block_kwargs):
        ...
        self.mlp = Mlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0.2)
...
def DiM_XL_16(**kwargs):
    return DiM(depth=28, hidden_size=1152, patch_size=16, **kwargs)
...
def DiM_B_16(**kwargs):
    return DiM(depth=12, hidden_size=768, patch_size=16, **kwargs)
...
DiM_models = {
    'DiM-XL/2': DiM_XL_2,  'DiM-XL/4': DiM_XL_4,  'DiM-XL/8': DiM_XL_8, 'DiM-XL/16': DiM_XL_16,
    'DiM-L/2':  DiM_L_2,   'DiM-L/4':  DiM_L_4,   'DiM-L/8':  DiM_L_8,
    'DiM-B/2':  DiM_B_2,   'DiM-B/4':  DiM_B_4,   'DiM-B/8':  DiM_B_8, 'DiM-B/16': DiM_B_16,
...
if __name__ == "__main__":
    ...
    inp = torch.randn(1, 4, latent_size, latent_size)
    t = torch.tensor([0])
    y = torch.tensor([0])

...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = np.array(Image.open(os.path.join(self.features_dir, feature_file)).convert('RGB'))
        labels = np.array(Image.open(os.path.join(self.labels_dir, label_file)).convert('RGB'))
        features = features.astype(np.float32) / 127.5 - 1
        labels = labels.astype(np.float32) / 127.5 - 1
        return torch.from_numpy(features), torch.from_numpy(labels), os.path.join(self.features_dir, feature_file), os.path.join(self.labels_dir, label_file)
...
def main(args):
    ...
    # Setup data:
    features_x_dir = f"{args.feature_path}/clean"
    features_y_dir = f"{args.feature_path}/adv"
    dataset = CustomDataset(features_x_dir, features_y_dir)
    loader = DataLoader(
        dataset,
    ...
    for epoch in range(args.epochs):
        ...
        for x, y, x_path, y_path in loader:
            x = x.permute(0, 3, 1, 2)
            y = y.permute(0, 3, 1, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature-path", type=str, default="dataPath/train")
    ...
    parser.add_argument("--epochs", type=int, default=1450)
